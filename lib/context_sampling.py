"""Context sampling utilities for TabPFN training and evaluation.

TabPFN uses attention over (context + query) rows, so memory grows quickly
with sequence length. These utilities chunk large datasets into manageable
context sizes for training and evaluation.

Adapted from TabPFN examples/scratch_training/context_sampling.py.
"""
...
def sample_train_context_indices(
    ...
    """Sample context_size row indices from n_train training rows."""
    if n_train < 2:
...
def predict_with_context_batched(
    *,
    ...
    """Fit on one context and predict the full test set in chunks (regression)."""
    ...
    for start in range(0, n_test, test_batch_size):
        ...
        X_batch = X_test[start:end] if not hasattr(X_test, "iloc") else X_test.iloc[start:end]
        out[start:end] = np.asarray(regressor.predict(X_batch), dtype=np.float32).reshape(-1)
    return out


def predict_proba_with_context_batched(
    *,
    classifier,
    X_ctx: Any,
    y_ctx: np.ndarray,
    ...
    """Fit on one context and predict_proba the full test set in chunks (classification).

    Returns:
        Array of shape (n_test, n_classes) with class probabilities.
    """
    if test_batch_size < 1:
        raise ValueError("test_batch_size must be >= 1.")
    ...
    n_test = len(X_test)
    n_classes = len(np.unique(y_ctx))
    out = np.empty((n_test, n_classes), dtype=np.float32)
    for start in range(0, n_test, test_batch_size):
        end = min(start + test_batch_size, n_test)
        X_batch = X_test[start:end] if not hasattr(X_test, "iloc") else X_test.iloc[start:end]
        out[start:end] = np.asarray(classifier.predict_proba(X_batch), dtype=np.float32)
    return out
