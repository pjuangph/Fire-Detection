"""realtime_tabpfn_regression.py - Real-time fire detection with TabPFN regressor.

Thin wrapper around lib.realtime that loads a TabPFN regression model from
a YAML config.

Usage:
    python realtime_tabpfn_regression.py --config configs/best_model.yaml
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Real-time fire detection simulation (TabPFN regression)')
    parser.add_argument(
    ...
    model_type = model_cfg.get('model_type', 'tabpfn_regression')
    ...
    if ml_model is None:
        print(f'ERROR: checkpoint not found: {model_path}',
              file=sys.stderr)
        sys.exit(1)
    print(f'Using TabPFN regressor fire detector ({args.config})')
    ...
    for fnum, info in sorted(flights.items()):
        simulate_flight(fnum, info['files'], info['comment'], gs,
                        ml_model=ml_model, detector_name='tabpfn_regression')
